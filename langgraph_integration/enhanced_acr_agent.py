...
class EnhancedACRAgent:
    ...
    async def _analyze_query_for_acr(self, user_query: str) -> Dict[str, Any]:
        ...
        **MEDICAL REASONING EXAMPLES:**
        ...
        Example 1: "Blood in stool, bright red"
        → Symptom Analysis: Bright red blood suggests lower GI source
        → Anatomical Deduction: Colon, rectum, or anus bleeding
        → Condition Translation: Lower gastrointestinal bleeding
        → ACR Search: "Lower gastrointestinal tract bleeding"
        
        Example 2: "Right upper quadrant pain + fever + elevated WBC"
        → Symptom Analysis: RUQ pain + inflammation markers
        → Anatomical Deduction: Biliary system involvement
        → Condition Translation: Acute biliary disease/cholangitis
        → ACR Search: "Acute biliary disease fever elevated WBC"
        
        Example 3: "Chest pain + shortness of breath"
        → Symptom Analysis: Cardiopulmonary symptoms
        → Anatomical Deduction: Heart/lung/vascular system
        → Condition Translation: Acute chest pain syndrome
        → ACR Search: "Acute chest pain shortness of breath"
        ...
        **CRITICAL INSTRUCTIONS:**
        1. **APPLY MEDICAL REASONING**: Use your medical training to analyze symptoms systematically
        2. **THINK ANATOMICALLY**: Every symptom points to specific body regions - identify them
        3. **TRANSLATE TO CONDITIONS**: Convert symptom clusters into formal medical diagnoses
        4. **USE MEDICAL TERMINOLOGY**: Employ precise anatomical and pathological terms
        5. **CONSIDER CLINICAL CONTEXT**: Determine if this is screening, diagnostic, or follow-up
        6. **MAP TO ACR STRUCTURE**: Format queries to match how ACR organizes conditions
        ...
        {{
            ...
            "acr_search_strategy": {{
                "primary_search_query": "condition-based query using formal medical terminology",
                "alternative_queries": ["anatomical backup query", "symptom-based backup query"],
        ...
        **PRIMARY QUERY STRATEGY:**
        - Start with your medical assessment of the most likely condition
        - Include specific anatomical region
        - Add clinical context (initial/follow-up/emergency/etc.)
        - Use precise medical terminology throughout
        
        **BACKUP QUERIES:**
        - Alternative 1: Different anatomical perspective
        - Alternative 2: Broader condition category
        
        **REMEMBER:** You have extensive medical training - use it to think like a physician analyzing a case.
        """
    ...
    async def _execute_acr_retrieval(self, user_query: str, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        retrieval_results = {
            ...
            "search_metadata": {
                ...
                "best_similarity": 0.0,
                "total_procedures": 0,
                "search_method": "direct_variant"
            }
        }
        
        try:
            # Primary search with optimized query using direct variant search
            search_strategy = analysis.get("acr_search_strategy", {})
            primary_query = search_strategy.get("primary_search_query", user_query)
            
            print(f"ACR Direct Variant Search: '{primary_query}'")
            primary_result = await self._direct_variant_search_with_context(primary_query)
            retrieval_results["primary_result"] = primary_result
            retrieval_results["search_metadata"]["queries_tried"].append(primary_query)
            
            # Track best similarity
            if "best_variant" in primary_result and "variant_similarity" in primary_result["best_variant"]:
                retrieval_results["search_metadata"]["best_similarity"] = primary_result["best_variant"]["variant_similarity"]
            
            # Count procedures
            if "usually_appropriate_procedures" in primary_result:
                retrieval_results["search_metadata"]["total_procedures"] = len(primary_result["usually_appropriate_procedures"])
            
            # Try alternative queries if primary result is weak
            primary_similarity = retrieval_results["search_metadata"]["best_similarity"]
            if primary_similarity < 0.7:  # Low similarity threshold
                ...
                for alt_query in alternative_queries[:2]:  # Limit to 2 alternatives
                    if alt_query != primary_query:
                        print(f"ACR Alternative Variant Search: '{alt_query}'")
                        alt_result = await self._direct_variant_search_with_context(alt_query)
                        retrieval_results["alternative_results"].append({
                            "query": alt_query,
                            "result": alt_result
                        })
                        retrieval_results["search_metadata"]["queries_tried"].append(alt_query)
                        
                        # Update best similarity if better
                        if "best_variant" in alt_result and "variant_similarity" in alt_result["best_variant"]:
                            alt_similarity = alt_result["best_variant"]["variant_similarity"]
                            if alt_similarity > retrieval_results["search_metadata"]["best_similarity"]:
                                retrieval_results["search_metadata"]["best_similarity"] = alt_similarity
            
            print(f"ACR Direct Variant Retrieval Complete: {len(retrieval_results['search_metadata']['queries_tried'])} queries tried")
            return retrieval_results
            
        except Exception as e:
            print(f"ACR direct variant retrieval failed: {e}")
            # Fallback to basic search
            fallback_result = await self._direct_variant_search_with_context(user_query)
            return {
                "primary_result": fallback_result,
                "alternative_results": [],
                "search_metadata": {
                    "queries_tried": [user_query],
                    "best_similarity": 0.0,
                    "total_procedures": 0,
                    "search_method": "direct_variant_fallback",
                    "error": str(e)
                }
            }
    ...
    async def _evaluate_acr_results(self, user_query: str, analysis: Dict[str, Any], acr_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        evaluation_prompt = f"""
You are a radiology decision-support assistant with deep knowledge of ACR Appropriateness Criteria.

CLINICAL QUERY (verbatim):
{user_query}

ACR SEARCH CANDIDATES (ranked by semantic similarity):
{formatted_variants}

TASK:
Analyse the clinical query and the candidate variants. Pick the single variant that BEST fits the query.
Focus on clinical phase (diagnosis vs treatment vs follow-up), anatomy, and overall scenario match.

EXPECTED CLINICAL PHASE (from query context): {expected_phase}
...
"""
        ...
        try:
            ...
            if should_override:
                ...
                print("✅ Keeping ColBERT rank-1 variant.")
                return self._add_evaluation_metadata(primary_result, analysis, acr_results, "llm_confirmed", llm_reasoning=reasoning, llm_confidence=confidence, original_rank=1)
    ...
    async def _rebuild_result_with_variant(self, original_result: Dict[str, Any], selected_variant: Dict[str, Any], query: str) -> Dict[str, Any]:
        ...
        try:
            # Get procedures for the selected variant
