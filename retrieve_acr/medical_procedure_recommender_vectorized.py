...
class MedicalProcedureRecommenderVectorized:
    ...
    def get_procedures_for_variant(self, variant_id: str, appropriateness_filter: str = None) -> List[Dict]:
        ...
        if appropriateness_filter:
            ...
            RETURN type(r) as appropriateness, p.id as procedure_id, 
        ...
        with self.driver.session() as session:
            ...
            for record in result:
                procedure_info = {
                    'procedure_id': record['procedure_id'],  # Use procedure_id to match old format
                    'title': record['procedure_id'],  # Use procedure_id as title since title property doesn't exist
                    'appropriateness': record['appropriateness'],
                    'dosage': record['dosage'],
                    'properties': dict(record['procedure_properties'])
                }
